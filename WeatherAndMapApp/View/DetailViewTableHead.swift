...
class DetailViewTableHead: UIView {
    ...
    var currentWeather: Current? {
        didSet {
            ...
            tempLabel.text = String(format: "%.1f", data.actualTemp) + "°C"
            weatherLabel.text = data.description.capitalizingFirstLetter()
            feelTempLabel.text = Localize.Detail.FeelsLike + String(format: "%.1f", data.sensoryTemp) + "°C"
        }
    }
